...
class WhisperNotes(QObject):
    ...
    def __init__(self, app):
        ...
        ollama_model = self.settings.value("ollama_model", "llama3")  # Default to llama3

        if journal_dir and os.path.isdir(journal_dir):
            self.journal_manager = JournalingManager(output_dir=journal_dir, summary_prompt=summary_prompt, ollama_model=ollama_model)
            ...
            self.journal_manager = JournalingManager(output_dir=default_journal_dir, summary_prompt=summary_prompt, ollama_model=ollama_model)
    ...
    def prompt_set_journal_dir(self):
        ...
        if journal_dir:
            ...
            # Update the journal manager with the new directory and existing settings
            summary_prompt = self.settings.value("summary_prompt")
            ollama_model = self.settings.value("ollama_model", "llama3")
            self.journal_manager = JournalingManager(output_dir=journal_dir, summary_prompt=summary_prompt, ollama_model=ollama_model)
    ...
    def prompt_edit_summary_prompt(self):
        ...
        default_prompt = "Summarize the following in 1-2 sentences from a first-person perspective, as if you are the person who said it:"
    ...
    def prompt_edit_format_prompt(self):
        ...
        default_prompt = "Add paragraph breaks and fix punctuation only. Do NOT change, rephrase, or rewrite any words. Keep the exact original wording:"
    ...
    def prompt_select_ollama_model(self):
        ...
        try:
            # Import ollama here to handle if it's not available
            import ollama
            ...
            for model in available_models:
                ...
                size = model.get('details', {}).get('parameter_size', 'unknown size')
                model_choices.append(f"{name} ({size})")
            ...
            for i, choice in enumerate(model_choices):
                if choice.startswith(current_model):
                    current_index = i
                    break
            ...
            from PySide6.QtWidgets import QComboBox
            dialog = QDialog(None)
            dialog.setWindowTitle("Select Ollama Model")
            dialog.setMinimumWidth(500)
            ...
            # Add explanation label
            explanation = QLabel(
                "Select which Ollama model to use for generating journal summaries and formatting.\n\n"
                "Larger models may provide better results but will be slower."
            )
            explanation.setWordWrap(True)
            layout.addWidget(explanation)
            ...
            model_combo.addItems(model_choices)
            model_combo.setCurrentIndex(current_index)
            layout.addWidget(model_combo)

            # Add current model info
            current_info = QLabel(f"Currently using: {current_model}")
            current_info.setStyleSheet("color: #666; font-style: italic;")
            layout.addWidget(current_info)
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                selected_model_full = model_choices[selected_index]
                # Extract just the model name (before the size in parentheses)
                selected_model = selected_model_full.split(' (')[0]

                # Save to settings
                self.settings.setValue("ollama_model", selected_model)
                logging.info(f"Ollama model changed to: {selected_model}")

                # Update the journal manager with the new model
                journal_dir = self.settings.value("journal_dir")
                summary_prompt = self.settings.value("summary_prompt")
                format_prompt = self.settings.value("format_prompt")

                if journal_dir and os.path.isdir(journal_dir):
                    self.journal_manager = JournalingManager(output_dir=journal_dir, summary_prompt=summary_prompt, ollama_model=selected_model)
                else:
                    home_dir = os.path.expanduser("~")
                    default_journal_dir = os.path.join(home_dir, "Documents", "Personal", "Audio Journal")
                    self.journal_manager = JournalingManager(output_dir=default_journal_dir, summary_prompt=summary_prompt, ollama_model=selected_model)
                ...
                self.tray_icon.showMessage(
                    "Settings Updated",
                    f"Ollama model set to: {selected_model}\n\nThis will be used for all future journal entries.",
                    QSystemTrayIcon.Information,
                    5000
                )
            ...
            QMessageBox.critical(
                ...
                "Ollama Python library is not installed. Please ensure Ollama is properly installed."
            )
        except Exception as e:
            logging.error(f"Error selecting Ollama model: {e}", exc_info=True)
            QMessageBox.critical(
                None,
                "Error",
    ...
    def _clear_transcriber_references(self):
        logging.info("[WhisperNotes] Clearing self.transcriber reference after it finished and deleteLater was called.")
        self.transcriber = None

    def _clear_transcription_thread_references(self):
        logging.info("[WhisperNotes] Clearing self.transcription_thread reference after it finished and deleteLater was called.")
        self.transcription_thread = None
